...
router.post('/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const collectionName = 'YourCollectionName'; // Define collection name
    const schema = new mongoose.Schema(fieldNames.reduce((acc, curr) => ({ ...acc, [curr]: String }), {}), { strict: false });
    const CollectionModel = mongoose.model(collectionName, schema);
